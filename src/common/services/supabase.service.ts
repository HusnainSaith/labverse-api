...
export class SupabaseService {   
  ...
  async uploadImage(file: Express.Multer.File, folderName: string): Promise<string> {
    try {
      ...
      SafeLogger.log(`Image uploaded successfully to: ${publicUrlData.publicUrl}`, 'SupabaseService');     
      return publicUrlData.publicUrl;   

    } catch (error) {
      SafeLogger.error(`Image upload failed: ${error.message}`, 'SupabaseService');
  ...
  async deleteImage(imageUrl: string): Promise<void> {
    try {
      ...
      if (error) {
        SafeLogger.error(`Failed to delete image: ${error.message}`, 'SupabaseService');
        throw new InternalServerErrorException('Failed to delete image from storage');
      }

      SafeLogger.log(`Image deleted successfully: ${fileName}`, 'SupabaseService');

    } catch (error) {
      SafeLogger.error(`Image deletion failed: ${error.message}`, 'SupabaseService');
