...
export class InvoicesService {
  ...
  async create(createInvoiceDto: CreateInvoiceDto): Promise<{ success: boolean; message: string; data: Invoice }> {
    if (createInvoiceDto.invoice_number) {
      ValidationUtil.validateString(createInvoiceDto.invoice_number, 'invoice_number', 3, 50);
    }
    ValidationUtil.validateDecimal(createInvoiceDto.total_amount, 'total_amount');
    ...
    if (createInvoiceDto.quotation_id) {
      ValidationUtil.validateUUID(createInvoiceDto.quotation_id, 'quotation_id');
    }
    ...
    if (createInvoiceDto.description) {
      ValidationUtil.validateString(createInvoiceDto.description, 'description', 0, 1000);
    }

    // Validate date logic
    if (new Date(createInvoiceDto.issue_date) > new Date(createInvoiceDto.due_date)) {
    ...
    try {
      const invoice = this.invoicesRepository.create({
        ...createInvoiceDto,
        invoice_number: createInvoiceDto.invoice_number ? ValidationUtil.sanitizeString(createInvoiceDto.invoice_number) : undefined,
        description: createInvoiceDto.description ? ValidationUtil.sanitizeString(createInvoiceDto.description) : undefined,
      });
      
      const savedInvoice = await this.invoicesRepository.save(invoice);
      SafeLogger.log(`Invoice created: ${createInvoiceDto.invoice_number || 'Auto-generated'}`, 'InvoicesService');
  ...
  async findAll(): Promise<{ success: boolean; message: string; data: Invoice[] }> {
  ...
  async findOne(id: string): Promise<{ success: boolean; message: string; data: Invoice }> {
  ...
  async update(
    ...
    if (updateInvoiceDto.invoice_number) {
      ValidationUtil.validateString(updateInvoiceDto.invoice_number, 'invoice_number', 3, 50);
    }
    if (updateInvoiceDto.total_amount !== undefined) {
      ValidationUtil.validateDecimal(updateInvoiceDto.total_amount, 'total_amount');
    }
    ...
    if (updateInvoiceDto.quotation_id) {
      ValidationUtil.validateUUID(updateInvoiceDto.quotation_id, 'quotation_id');
    }
    ...
    if (updateInvoiceDto.description !== undefined) {
      if (updateInvoiceDto.description) {
        ValidationUtil.validateString(updateInvoiceDto.description, 'description', 0, 1000);
      }
    }
    ...
    try {
      ...
      if (newIssueDate && newDueDate && new Date(newIssueDate) > new Date(newDueDate)) {
        throw new BadRequestException('Issue date cannot be after due date');
      }
      ...
      const updateData = {
        ...
        ...(updateInvoiceDto.invoice_number && { 
          invoice_number: ValidationUtil.sanitizeString(updateInvoiceDto.invoice_number) 
        }),
        ...(updateInvoiceDto.description !== undefined && { 
          description: updateInvoiceDto.description ? ValidationUtil.sanitizeString(updateInvoiceDto.description) : null 
        }),
      };
