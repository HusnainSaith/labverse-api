...
export class DevelopmentPlanServicesController {
  ...
  @ApiResponse({ status: HttpStatus.CREATED, description: 'Association created.', type: CreateDevelopmentPlanServiceDto })
  async create(@Body() createDpsDto: CreateDevelopmentPlanServiceDto) {
    return this.dpsService.create(createDpsDto);
  }

  @Get()
  @ApiOperation({ summary: 'Retrieve all development plan service associations' })
  @ApiResponse({ status: HttpStatus.OK, description: 'Successfully retrieved all associations.', type: [CreateDevelopmentPlanServiceDto] })
  async findAll() {
    return this.dpsService.findAll();
  }

  @Get(':id')
  @ApiOperation({ summary: 'Retrieve a development plan service association by ID' })
  @ApiResponse({ status: HttpStatus.OK, description: 'Association found.', type: CreateDevelopmentPlanServiceDto })
  ...
  @ApiResponse({ status: HttpStatus.OK, description: 'The association has been successfully updated.', type: CreateDevelopmentPlanServiceDto })
  @ApiResponse({ status: HttpStatus.NOT_FOUND, description: 'Association not found.' })
  async update(@Param('id') id: string, @Body() updateDpsDto: UpdateDevelopmentPlanServiceDto) {
