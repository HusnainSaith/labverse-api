...
export class MessagingService {
  ...
  async createConversation(createConversationDto: CreateConversationDto): Promise<Conversation> {
    ...
    if (participantUserIds && (!Array.isArray(participantUserIds) || participantUserIds.length === 0)) {
      throw new BadRequestException('`participantUserIds` must be a valid array of user IDs.');
    }

    if (!conversationData.isGroupChat && participantUserIds && participantUserIds.length > 2) {
  ...
  async deleteConversation(conversationId: string): Promise<{ message: string }> {
    ...
    await this.participantRepository.delete({ conversation: { id: conversationId } });
